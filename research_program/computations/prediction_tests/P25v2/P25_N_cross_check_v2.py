...
print("-" * 70)
Omega_ratio = 5.0  # Omega_DM / Omega_B ~ 5
k_eff = 1 / (Omega_ratio + 1)
print(f"Omega_DM / Omega_B = {Omega_ratio}")
print(f"k_eff = Omega_B / (Omega_DM + Omega_B) = 1 / (5 + 1) = {k_eff:.4f}")
print()
...
if N_DA == N_AQN == N_HaPPY:
    ...
    print(f"  The ratio Omega_DM/Omega_B ~ 5 is the ratio of dark to visible sectors.")
    print(f"  This is a GEOMETRIC EXPLANATION of the coincidence Omega_DM/Omega_B ~ 5.")
else:
    print(f"\n[FAIL] INCONSISTENCY: {N_DA}, {N_AQN}, {N_HaPPY}")
    print(f"  The synthesis has an internal tension.")

# Additional check: what would different Omega_DM/Omega_B predict for N?
print("\n" + "=" * 70)
print("SENSITIVITY: What if Omega_DM/Omega_B were different?")
print("=" * 70)
for ratio in [3, 4, 4.5, 5, 5.5, 6, 7, 10]:
    N_pred = int(round(ratio + 1))
    print(f"  Omega_DM/Omega_B = {ratio:4.1f} -> N = {N_pred}")
print(f"\n  Observed: Omega_DM/Omega_B = {5.36:.2f} (Planck 2018: Omega_DM=0.265, Omega_B=0.0494)")
print(f"  -> N = {round(5.36 + 1)} = 6 (nearest integer)")
print(f"  Precision: Omega_DM/Omega_B = 5.36 vs predicted 5.00 (for N exactly 6)")
