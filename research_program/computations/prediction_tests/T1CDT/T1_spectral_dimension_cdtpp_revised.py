...
def spectral_dimension_laplacian(adj: Adjacency, n_simplices: int, dimension: int = 3,
    ...
    # Approximate trace using available lowest modes. Good for IR / intermediate scales,
    # not trustworthy deep UV unless k is large. We expose this limitation explicitly.
    K_vals = np.array([np.mean(np.exp(-vals * t)) for t in t_vals], dtype=float)
    ds = estimate_ds_from_curve(t_vals, K_vals)

    if not quiet:
        uv_band = float(np.nanmean(ds[6:14]))
        ir_band = float(np.nanmean(ds[-14:-6]))
        print(f"  d_s at intermediate-small t: {uv_band:.2f} (CDT UV target ~2, but only if enough modes are included)")
        print(f"  d_s at large t: {ir_band:.2f} (expected ~{dimension})")
        if k < min(2000, N - 2):
            print("  [NOTE] UV estimate is mode-truncated and should NOT be trusted quantitatively yet.")
