# P8: NANOGrav 15-yr Beat-Frequency Modulation Test

## Prediction
CCDR predicts a periodic modulation of the gravitational-wave background
sensitivity in log-frequency space, arising from the cosmic crystal grain
spacing. The modulation should appear as a sinusoidal residual on top of
the smooth power-law sensitivity curve.

**The critical test is NOT just measuring an oscillation amplitude** —
it's comparing against a null hypothesis (synthetic curves with no beats).

## Hardware
Any modern laptop. ~5 min runtime.
...
## Data
**NANOGrav 15-year sensitivity curves (public, Zenodo):**
- DOI: 10.5281/zenodo.8092346
- Direct: https://zenodo.org/records/8092346/files/NANOGrav15yr_Sensitivity-Curves_v1.0.0.tar.gz
- Reference: Agazie et al. 2023, ApJL 951, L9
...
The v1 script computed amp_log = 2.0 and reported it as a "result" without
asking: is this big or small? With NO null comparison, the number is
meaningless — the NANOGrav curve has intrinsic structure (telescope
coverage, calibration regime transitions) that produces oscillatory
residuals even in the absence of any physical signal.

The v2 script generates 1000 synthetic null curves with the same overall
shape but no beat structure, fits the same model to each, and compares
the real amp_log against the null distribution. Only if the real
measurement is >3σ above the null can a detection be claimed.

## Expected Result

Most likely: amp_log will be in the bulk of the null distribution
(p > 0.05, σ < 2). The "amp_log = 2.0" from v1 is almost certainly
fitting the curve's intrinsic structure, not a CCDR signal.

If amp_log IS significantly above null, the next step is to check
whether the period (omega) corresponds to a physically meaningful
crystal scale.
