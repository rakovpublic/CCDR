# P8b: GW Background Spectral Index Test

## Prediction
The standard prediction for a gravitational-wave background from
supermassive black hole binaries (SMBHBs) is a power-law characteristic
strain spectrum:

```
h_c(f) ∝ f^α   where α = -2/3
```

equivalent to a timing residual power spectrum index γ = 13/3 ≈ 4.33
(in NANOGrav's parameterisation: γ = 3 - 2α).

**The CCDR claim:** if the cosmic crystal carries running vacuum
dynamics with coefficient ν ~ 10⁻³, the GW background spectrum
should pick up a small modification:

```
α_CCDR = -2/3 + κ·ν
```

where κ is an O(1) constant from the synthesis derivation. For κ = 1
and ν = 10⁻³ this gives α_CCDR ≈ -0.666, a shift of ~10⁻³ from the
standard value.

In NANOGrav's parameterisation:

```
γ_CCDR = 13/3 - 2κ·ν ≈ 4.331  (vs standard 13/3 = 4.333)
```

**Why this is structural:** the ν coefficient appears in P01 (the
primary CCDR prediction from DESI). It's not a free parameter — it's
determined by the GFT condensate structure in Layer 1. So this test
links Layer 5 (cosmology) to Layer 1 (quantum gravity) through a
single common parameter.

## Honesty caveat
The current NANOGrav 15-yr measurement is γ = 3.2 ± 0.6 (Agazie+ 2023),
with large uncertainty. The predicted shift (~10⁻³) is FAR below the
current precision. So this test can only check whether γ is *consistent*
with the prediction, not whether it confirms it.
...
### NANOGrav 15-year main result paper
**Agazie et al. 2023, ApJL 951, L8**
- arXiv: https://arxiv.org/abs/2306.16213
- The published value: γ = 3.2 ± 0.6 (Table 1, free spectral index fit)
- For the SMBHB-fixed analysis: γ = 13/3 (fixed)
- Posterior data: https://zenodo.org/records/8092346

### Reference values (from the paper)
- A_GWB (amplitude at f = 1 yr⁻¹): A = 6.4 × 10⁻¹⁵
- γ (free fit): 3.2 ± 0.6 (1σ)
- γ (SMBHB fixed): 13/3 ≈ 4.333
...
> CONSISTENT (within 1σ) — but uncertainty too large to discriminate

The CCDR prediction (γ ≈ 4.331) and the SMBHB prediction (γ = 13/3 ≈ 4.333)
both lie ~1.9σ above the NANOGrav central value (γ = 3.2). They are both
"consistent" with the data, but the data cannot distinguish between them.

## What This Achieves for Layer 5

This test is the WEAKEST of the three options. It only demonstrates that:

1. The CCDR prediction is not yet falsified
2. The required precision is far beyond current PTA capability
3. The link to Layer 1 (via the ν parameter) is mathematically sound

The honest framing: "P8b is a placeholder. The CCDR spectral index
prediction cannot be tested with current data, and may not be testable
with any near-future PTA. It serves only to record the formal link
between the cosmological prediction and the GFT condensate parameter ν."

## Why Include It At All

Even though the test is weak, it has value because:
- It documents the prediction explicitly so it's falsifiable in principle
- It records the link to ν, which IS testable via P01 (DESI)
- It prevents the synthesis from later "remembering" a different value
