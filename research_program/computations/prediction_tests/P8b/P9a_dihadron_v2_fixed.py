...
def fetch_url(url: str, timeout: int = 60, max_redirects: int = 10) -> bytes:
    """Fetch a URL and manually follow redirects, including HTTP 308."""
    current = url
...
def _extract_numeric_value(entry: dict) -> Optional[float]:
...
def load_hepdata_projection(url: str, table: Optional[str] = None, fmt: str = "json") -> Tuple[np.ndarray, np.ndarray, Optional[np.ndarray], dict]:
    ...
    if "values" in data and isinstance(data["values"], list):
        ...
        for row in data["values"]:
            ...
            if not x_list or not y_list:
                continue
            x_val = _extract_numeric_value(x_list[0])
            y_val = _extract_numeric_value(y_list[0])
            if x_val is None or y_val is None:
                continue
            phi_vals.append(float(x_val))
            corr_rows.append(y_list[0])

        if not phi_vals:
            raise RuntimeError("Could not extract x/y values from compact HEPData payload")

        phi = np.asarray(phi_vals, dtype=float)
        corr = _pick_first_numeric_values(corr_rows)
        err = _pick_errors(corr_rows)
        headers = data.get("headers") or []
        x_name = headers[0].get("name") if len(headers) > 0 and isinstance(headers[0], dict) else None
        y_name = headers[1].get("name") if len(headers) > 1 and isinstance(headers[1], dict) else None
        meta = {
            "source": "hepdata",
            "download_url": download_url,
    ...
    raise RuntimeError(
        "Unexpected HEPData payload structure. Supported layouts are "
        "independent_variables/dependent_variables and compact headers/values[x/y]."
    )
