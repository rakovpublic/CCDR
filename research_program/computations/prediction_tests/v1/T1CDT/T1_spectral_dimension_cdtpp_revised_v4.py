#!/usr/bin/env python3
"""
T1_spectral_dimension_cdtpp_revised_v4.py
...
Usage:
  python3 T1_spectral_dimension_cdtpp_revised_v4.py <data_dir_or_file> [dimension]

Outputs:
  - T1_cdtpp_revised_v4_results.npz
  - T1_cdtpp_revised_v4_spectral_dimension.png
"""
...
def run_sanity_suite(rng: np.random.Generator) -> SanityResult:
    ...
    if SCIPY_AVAILABLE:
        try:
            # IMPORTANT: for bounded finite-degree graphs, the heat-trace estimate built from the
            # graph random-walk Laplacian is NOT a valid UV dimensional sanity check. As t->0 the
            # finite spectrum makes d_s(t)->0 rather than the continuum dimension. Therefore we do
            # not hard-fail if the Laplacian estimator does not return ~3 on a finite cubic torus.
            # We only require that the computation runs and yields a finite, nonnegative IR curve.
            t_lap, ds_lap, _ = spectral_dimension_laplacian(lattice, len(lattice), dimension=3,
                                                            n_eigenvalues=min(512, len(lattice) - 2),
                                                            lazy=True, quiet=True)
            if ds_lap is None or t_lap is None:
                return SanityResult(False, "sanity Laplacian failed to compute")
            finite = np.isfinite(ds_lap)
            if finite.sum() < 10:
                return SanityResult(False, "sanity Laplacian returned too few finite points")
            lap_ir = float(np.nanmean(ds_lap[-12:-4]))
            lap_min = float(np.nanmin(ds_lap[finite]))
            lap_max = float(np.nanmax(ds_lap[finite]))
            print(f"  3D cubic torus Laplacian sanity: IR mean d_s ≈ {lap_ir:.2f}; range=[{lap_min:.2f}, {lap_max:.2f}]")
            if lap_min < -0.25:
                return SanityResult(False, f"sanity Laplacian failed: strongly negative d_s values ({lap_min:.2f})")
            if lap_max > 6.5:
                return SanityResult(False, f"sanity Laplacian failed: implausibly large d_s values ({lap_max:.2f})")
            print("  [NOTE] Laplacian sanity on finite graphs is treated as an IR/numerical consistency check, not a UV dimension check.")
...
def spectral_dimension_laplacian(adj: Adjacency, n_simplices: int, dimension: int = 3,
    ...
    # IMPORTANT: on bounded finite-degree graphs this estimator is reliable only for IR / coarse
    # scales. It does NOT reproduce the continuum UV dimension as t->0; instead d_s(t) tends
    # toward 0 because the graph spectrum is bounded. Therefore treat this as an IR cross-check.
    t_vals = np.logspace(-1, 2, 64)
    ...
    if not quiet:
        mid_band = float(np.nanmean(ds[10:22]))
        ir_band = float(np.nanmean(ds[-14:-6]))
        print(f"  d_s at intermediate t: {mid_band:.2f} (diagnostic only; not a UV claim)")
        print(f"  d_s at large t: {ir_band:.2f} (expected ~{dimension})")
        print("  [NOTE] For finite graphs, Laplacian/heat-trace d_s(t) is an IR cross-check only.")
        if k < min(2000, N - 2):
            print("  [NOTE] This Laplacian run is also low-mode truncated: trust only broad IR consistency.")
...
def compare_estimators(sigmas: np.ndarray, ds_rw: np.ndarray, t_vals: Optional[np.ndarray],
                       ds_lap: Optional[np.ndarray], dimension: int,
                       n_simplices: int, n_eigenvalues: Optional[int] = None) -> Tuple[bool, str, str]:
    """
    Return (ok, summary, status).

    status in {"validated", "rw_only", "failed"}.

    Critical change vs earlier revisions:
      - A low-mode Laplacian estimate on a finite graph is treated as a diagnostic cross-check,
        not a hard veto, unless coverage is high enough to make the Laplacian estimate meaningfully
        constraining.
    """
    valid_rw = np.isfinite(ds_rw)
    if valid_rw.sum() < 6:
        return False, "RW estimator has too few valid points", "failed"

    rw_ir = float(np.nanmean(ds_rw[np.where(valid_rw)[0][-5:]]))
    parts = [f"RW IR≈{rw_ir:.2f}"]
    if rw_ir < max(1.5, dimension - 1.4) or rw_ir > dimension + 1.4:
        return False, f"random-walk IR dimension implausible: {rw_ir:.2f}", "failed"

    if ds_lap is None or t_vals is None:
        return True, ", ".join(parts + ["no Laplacian cross-check"]), "rw_only"

    lap_ir = float(np.nanmean(ds_lap[-14:-6]))
    parts.append(f"Lap IR≈{lap_ir:.2f}")

    coverage = None
    if n_eigenvalues is not None and n_simplices > 0:
        coverage = float(n_eigenvalues) / float(n_simplices)
        parts.append(f"eig-coverage≈{coverage:.3f}")

    # Only let the Laplacian veto the run when the graph is fairly large AND the spectral coverage
    # is substantial. Otherwise treat it as diagnostic-only because low-mode truncation can depress
    # the apparent IR dimension on finite graphs.
    lap_strict = (coverage is not None and coverage >= 0.25 and n_simplices >= 20000)

    if not np.isfinite(lap_ir):
        if lap_strict:
            return False, f"Laplacian IR invalid under strict regime", "failed"
        return True, ", ".join(parts + ["Laplacian inconclusive (finite/truncated)"]), "rw_only"

    if lap_strict:
        if lap_ir < max(1.5, dimension - 1.4) or lap_ir > dimension + 1.4:
            return False, f"Laplacian IR dimension implausible under strict regime: {lap_ir:.2f}", "failed"
        if abs(rw_ir - lap_ir) > 1.0:
            return False, f"estimators disagree too much under strict regime (RW {rw_ir:.2f} vs Lap {lap_ir:.2f})", "failed"
        return True, ", ".join(parts + ["strict Laplacian cross-check passed"]), "validated"

    # Diagnostic-only regime: do not veto, but annotate loudly.
    note = "Laplacian diagnostic only (finite/truncated spectrum)"
    if abs(rw_ir - lap_ir) > 1.0:
        note += "; estimator mismatch not used as veto"
        return True, ", ".join(parts + [note]), "rw_only"
    return True, ", ".join(parts + [note]), "rw_only"
...
def main() -> None:
    if len(sys.argv) < 2:
        print("Usage: python3 T1_spectral_dimension_cdtpp_revised_v4.py <data_dir_or_file> [dim]")
        sys.exit(1)
    ...
    np.savez(
        ...
        valid_pipeline=(pipeline_status != "failed"),
        pipeline_status=pipeline_status,
        validity_reason=reason,
    )
    print("\nResults saved: T1_cdtpp_revised_v4_results.npz")
    ...
    try:
        ...
        plt.tight_layout()
        plt.savefig("T1_cdtpp_revised_v4_spectral_dimension.png", dpi=160)
        print("Plot saved: T1_cdtpp_revised_v4_spectral_dimension.png")
    except Exception as exc:
