...
def _detect_columns(header_line: str) -> dict[str, int]:
    """
    Detect the Pantheon+ column layout.

    The public file currently exposes a header like:
      CID IDSURVEY zHD ... m_b_corr m_b_corr_err_DIAG MU_SH0ES MU_SH0ES_ERR_DIAG ...
    and that header may or may not be commented out.
    """
...
def load_pantheon_data(dat_path: Path, cov_path: Path, *, jitter: float = 1e-10) -> PantheonData:
    ...
    with dat_path.open("r", encoding="utf-8", errors="replace") as f:
        for line in f:
            ...
            try:
                if {"z", "mu", "mu_err"}.issubset(colmap):
                    z = float(parts[colmap["z"]])
                    mu = float(parts[colmap["mu"]])
                    mu_err = float(parts[colmap["mu_err"]])
                else:
                    # Conservative fallback for older simplified mock tables.
                    z = float(parts[1])
                    mu = float(parts[3])
