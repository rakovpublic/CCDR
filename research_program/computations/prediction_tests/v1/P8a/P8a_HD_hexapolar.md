# P8a: Hellings-Downs Hexapolar (l=6) Test

## Prediction
If the cosmic crystal has C₆ᵥ symmetry (the hexagonal Brillouin zone
hypothesis that produces the lepton Koide formula Q = 2/3), then the
gravitational-wave background must inherit a hexapolar (l = 6) angular
component on top of the standard quadrupolar Hellings-Downs curve.

The hexapolar amplitude relative to the standard quadrupole is set
by the crystal's chiral asymmetry:

```
A_hex / A_quad ~ ν × δ_CP ~ 10⁻³ × 10⁻³ ~ 10⁻⁶
```

where ν is the RVM running vacuum coefficient and δ_CP is the
CDT-derived CP asymmetry.

**Why this is structural, not extrapolated:** the C₆ᵥ symmetry is
required by the synthesis to produce the lepton Koide formula. Either
C₆ᵥ is real (and a hexapolar signature must exist somewhere in the GW
background) or C₆ᵥ is wrong (and the lepton Koide is a coincidence).
This test couples the cosmological prediction directly to the particle
physics prediction.

## Hardware
Any laptop. ~3 minutes.
...
### NANOGrav 15-year angular correlation
**Agazie et al. 2023, ApJL 951, L8** ("The NANOGrav 15-yr Data Set:
Evidence for a Gravitational-wave Background")
- arXiv: https://arxiv.org/abs/2306.16213
- NANOGrav data products: https://nanograv.org/science/data/
- Direct Zenodo deposit: https://zenodo.org/records/8092346

The relevant data is the binned Hellings-Downs angular correlation
function (Figure 1c of Agazie+ 2023) and the multipole decomposition
in their Table S1 (supplementary).

### NANOGrav 15-yr pulsar pair correlations
The full pair-by-pair cross-correlation values are released in the
NANOGrav 15-yr data products on Zenodo. Look for:
- `15yr_cw_analysis.tar.gz` or
- `NANOGrav15yr_v1.tar.gz`
...
The standard HD curve has a strong l=2 component (quadrupole, the
defining feature) with smaller l=4, 6, 8 contributions from the
finite resolution of the angular binning. With ~15 angular bins and
~30 pulsars, NANOGrav 15-yr can constrain the l=6 component to
about 10⁻²-10⁻¹ relative to l=2.

The CCDR prediction (10⁻⁶) is far below this sensitivity, so the
expected outcome is:

> CONSISTENT (CCDR prediction below current sensitivity)

This is the most we can hope for from current data. A real test
would need NANOGrav 20-yr (~2027) or SKA-PTA (~2030).

## What This Achieves for Layer 5

The original P8 was falsified at 0.96σ. P8a converts this into:

- **Layer 5 hexapolar prediction:** consistent within current limits
- **Future falsifiability:** SKA-PTA can detect or exclude the 10⁻⁶ level
- **Structural link:** the C₆ᵥ → hexapolar prediction is tied to the
  same symmetry that gives Q_leptons = 2/3, so success or failure
  here directly affects the lepton Koide interpretation

The crucial difference from the original P8: this prediction comes
from a structural feature the synthesis already requires, not from
extrapolation.
