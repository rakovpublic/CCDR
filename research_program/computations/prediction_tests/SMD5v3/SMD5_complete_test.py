...
def main():
    ...
    if eight_ninths_in_95:
        print(f"  [PASS] Q_up = 8/9 is CONSISTENT with current data")
    else:
        print(f"  [FAIL] Q_up = 8/9 is EXCLUDED at 95% CL")
    ...
    if three_fourths_in_95:
        print(f"  [PASS] Q_down = 3/4 is CONSISTENT with current data")
    else:
        print(f"  [FAIL] Q_down = 3/4 is EXCLUDED at 95% CL")
    ...
    if ratio_in_ci:
        print(f"  [PASS] δQ_up/δQ_down = 2C_F CONSISTENT")
    else:
    ...
    if sig_up < sig_needed_up:
        print(f"    [PASS] Current precision SUFFICIENT to distinguish 8/9 from 17/19")
    else:
        print(f"    [FAIL] Need {sig_up / sig_needed_up:.0f}× better precision")

    # What m_u precision gives sufficient Q precision?
    # Q_up ≈ m_t / (√m_t)^2 = dominated by m_t, but sensitivity to m_u
    # is through √m_u. δQ/δm_u ≈ (1 - Q·2/√m_u·S) / S^2 where S = sum of √m
    # Numerically:
    print(f"\n    Sensitivity to m_u:")
    ...
    if sig_down < sig_needed_down:
        print(f"    [PASS] Current precision SUFFICIENT")
    else:
        print(f"    [FAIL] Need {sig_down / sig_needed_down:.0f}× better precision")
