...
LZ_HEAVY_SI_JSON = "https://www.hepdata.net/record/155464?format=json"
LZ_LIGHT_SI_JSON = "https://www.hepdata.net/record/167778?format=json"

PANDAX_FIRST_ANALYSIS_XLSX = (
    "https://static.pandax.sjtu.edu.cn/pandax/DataRelease/"
    "PandaX-4T/first_analysis/PandaX4T_Data_ne.xlsx"
)
PANDAX_FIRST_ANALYSIS_EFF_ROOT = (
    "https://static.pandax.sjtu.edu.cn/pandax/DataRelease/"
    "PandaX-4T/first_analysis/eff_RDQ_graph.root"
)
PANDAX_RUN0_CSV = (
    "https://static.pandax.sjtu.edu.cn/pandax/DataRelease/"
    "PandaX-4T/light_dm_259day/run0_data.csv"
)
PANDAX_RUN1_CSV = (
    "https://static.pandax.sjtu.edu.cn/pandax/DataRelease/"
    "PandaX-4T/light_dm_259day/run1_data.csv"
)
PANDAX_S2ONLY_ZIP = (
    "https://static.pandax.sjtu.edu.cn/pandax/DataRelease/"
    "PandaX-4T/run0_S2_only/s2only_data_release.zip"
)
...
def load_lz(cache_dir: Path) -> tuple[list[Curve], list[str]]:
    ...
    skipped: list[str] = []
    for name, url in [
        ("LZ_4.2tyr_SI", LZ_HEAVY_SI_JSON),
        ("LZ_5.7tyr_light_SI", LZ_LIGHT_SI_JSON),
    ]:
        try:
            curve = parse_hepdata_url(url, experiment="LZ", source_name=name)
            if curve is None:
                skipped.append(f"{name}: no usable SI curve found")
            else:
                curves.append(curve)
...
def load_pandax(cache_dir: Path) -> tuple[list[Curve], list[str]]:
    ...
    skipped: list[str] = []
    urls = [
        ("PandaX4T_first_analysis_xlsx", PANDAX_FIRST_ANALYSIS_XLSX),
        ("PandaX4T_lightDM_run0", PANDAX_RUN0_CSV),
        ("PandaX4T_lightDM_run1", PANDAX_RUN1_CSV),
        # downloaded for completeness; may not be directly used in this analysis
    ...
    for name, url in urls:
        try:
            ...
            local = download(url, cache_dir / f"{name}{ext}")
            if local.suffix.lower() == ".zip":
                extract_dir = cache_dir / f"{name}_unzipped"
                if not extract_dir.exists():
                    with zipfile.ZipFile(local) as zf:
                        zf.extractall(extract_dir)
                found = []
                for fp in extract_dir.rglob("*"):
                    if fp.is_file() and fp.suffix.lower() in {".csv", ".txt", ".tsv", ".xlsx", ".xls", ".json", ".yaml", ".yml"}:
                        found.extend(parse_tabular_file(fp, "PandaX", f"{name}/{fp.name}"))
                best = choose_best(found)
                if best is not None:
                    curves.append(best)
                else:
                    skipped.append(f"{name}: downloaded but no usable curve found")
            elif local.suffix.lower() in {".root"}:
                skipped.append(f"{name}: downloaded ROOT efficiency file but no direct mass-limit curve extracted")
            else:
                found = parse_tabular_file(local, "PandaX", name)
                best = choose_best(found)
                if best is not None:
                    curves.append(best)
                else:
...
def load_xenonnt(cache_dir: Path) -> tuple[list[Curve], list[str]]:
    ...
    try:
        zip_path = download(XENONNT_LIGHT_WIMP_ZIP, cache_dir / f"{name}.zip")
        ...
        for fp in extract_dir.rglob("*"):
            ...
            if fp.suffix.lower() not in {".csv", ".txt", ".tsv", ".xlsx", ".xls", ".json", ".yaml", ".yml"}:
                continue
            if not any(k in low for k in ["limit", "limits", "wimp", "si"]):
                continue
            if any(k in low for k in ["mirror", "notebook", "readme"]):
                continue
            candidate_files.append(fp)
        parsed: list[Curve] = []
        for fp in candidate_files:
            parsed.extend(parse_tabular_file(fp, "XENONnT", f"{name}/{fp.name}"))
        # prefer curves with SI/WIMP in source name
        parsed = sorted(parsed, key=lambda c: (("si" in c.source_name.lower()) + ("wimp" in c.source_name.lower()), c.mass_gev.size), reverse=True)
