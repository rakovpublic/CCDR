...
import numpy as np
try:
    from scipy.special import sph_harm_y as _scipy_sph_harm_new
except ImportError:  # SciPy < 1.15 / legacy path
    _scipy_sph_harm_new = None

try:
    from scipy.special import sph_harm as _scipy_sph_harm_old
except ImportError:  # SciPy >= 1.17 may remove this
    _scipy_sph_harm_old = None


def sph_harm_compat(m: int, ell: int, lon: np.ndarray, theta: np.ndarray) -> np.ndarray:
    """Compatibility wrapper across old/new SciPy spherical-harmonic APIs.

    Legacy scipy.special.sph_harm signature:
        sph_harm(m, n, theta_azimuth, phi_polar)

    New scipy.special.sph_harm_y signature:
        sph_harm_y(n, m, theta_polar, phi_azimuth)

    This wrapper preserves the old call pattern used in this script:
        sph_harm_compat(m, ell, lon_azimuth, theta_polar)
    """
    if _scipy_sph_harm_new is not None:
        return _scipy_sph_harm_new(ell, m, theta, lon)
    if _scipy_sph_harm_old is not None:
        return _scipy_sph_harm_old(m, ell, lon, theta)
    raise ImportError(
        "Neither scipy.special.sph_harm_y nor scipy.special.sph_harm is available. "
        "Install a supported SciPy version."
    )
...
def spherical_harmonic_design_matrix(theta: np.ndarray, lon: np.ndarray, lmax: int) -> Tuple[List[Tuple[int, int]], np.ndarray]:
    ...
    cols = [sph_harm_compat(m, ell, lon, theta) for ell, m in keys]
...
def reconstruct_from_alm(
    ...
    for (ell, m), coeff in alm.items():
        if ell < ell_min or ell > ell_max:
            continue
        out += coeff * sph_harm_compat(m, ell, lon, theta)
    return np.real(out)
