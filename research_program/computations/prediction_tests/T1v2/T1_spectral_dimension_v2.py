...
Key fixes from v1:
  ...
  - Validation: d_s must -> 2 at large sigma before oscillation search
...
class CDT2D_Proper:
    ...
    def spectral_dimension(self, sigma_max=200, n_points=30):
        """Compute d_s(sigma) with proper error handling."""
        ...
        # Numerical derivative: d_s = -2 d(ln P) / d(ln sigma)
...
def run_T1_corrected(N_configs=500, T=16, N_spatial=20):
    ...
    FIRST: validate that d_s -> 2 at large sigma.
    ...
    # VALIDATION: check d_s at large sigma
    print("\nVALIDATION: checking d_s -> 2 at large sigma...")
    sigmas_val, ds_val, P_val = cdt.spectral_dimension(sigma_max=100)

    # d_s at the largest sigma values (last 5 points)
    ds_large = ds_val[-5:]
    ds_large_mean = np.mean(ds_large[np.isfinite(ds_large)])

    print(f"  <d_s> at large sigma: {ds_large_mean:.3f}")
    print(f"  Expected: ~2.0 for 2D CDT")

    if abs(ds_large_mean - 2.0) > 1.0:
        print(f"\n  [FAIL] VALIDATION FAILED: d_s = {ds_large_mean:.3f}, expected ~2.0")
        ...
        print(f"  [PASS] VALIDATION PASSED: d_s ~ {ds_large_mean:.2f}")
    ...
    print(f"\n  Ensemble <d_s> at large sigma: {ds_ensemble_mean:.3f}")

    if abs(ds_ensemble_mean - 2.0) > 0.5:
        print(f"  [WARN] ENSEMBLE VALIDATION FAILED")
    ...
    print(f"<d_s> at large sigma: {ds_ensemble_mean:.3f} (should be ~2.0)")
    ...
    print(f"Significance of residuals: {significance:.1f} sigma")
    print(f"Time: {elapsed:.0f}s")

    if abs(ds_ensemble_mean - 2.0) > 0.5:
        print(f"\n[WARN] <d_s> = {ds_ensemble_mean:.2f} != 2.0")
        ...
        if abs(delta_d - 1e-3) / 1e-3 < 1:
            print(f"  Consistent with nu ~ 10^-3!")
        else:
            print(f"  But delta_d = {delta_d:.2e} != 10^-3 = target")
    else:
        print(f"\n[INSUFFICIENT] No significant oscillation at current sensitivity")
        if mean_err > 0:
            configs_needed = int(len(all_ds) * (3 / max(significance, 0.1))**2)
            print(f"  Need ~{configs_needed} configs for 3 sigma detection")
    ...
    np.savez('T1_results_v2.npz',
             ...
             N_configs=len(all_ds),
             validation_passed=abs(ds_ensemble_mean - 2.0) < 0.5)
    ...
    try:
        ...
        if ds_smooth is not None:
            axes[0].plot(sigma_ref[valid], ds_smooth, 'r-', label='Smooth fit')
        axes[0].axhline(2.0, color='green', linestyle='--', alpha=0.5,
                        label='d_s = 2')
        ...
        axes[1].set_title(f'delta_d = {delta_d:.6f}, '
...
if __name__ == '__main__':
    ...
    # If validation passes, will scale to 500 configs
