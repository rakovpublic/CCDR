...
def _download_static(names: list[str] | None = None) -> list[Path]:
    out = []
...
def main() -> None:
    ...
    verdict = "support_like_high_k_side" if high_k_peak is not None and high_k_peak >= 1000 else "no_support_or_no_numeric_hbn_tables"
    status = "ok" if hbn_tables else "source_available_but_no_numeric_tables"
