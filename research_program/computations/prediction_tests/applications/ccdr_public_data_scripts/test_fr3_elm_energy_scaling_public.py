...
from _common_public_data import DEFAULT_HEADERS, cached_download, ensure_dir, fit_linear, json_dump, structured_report
...
def _mast_gate() -> dict:
    try:
        r = requests.get(API, headers=DEFAULT_HEADERS, timeout=60)
        r.raise_for_status()
        data = r.json()
