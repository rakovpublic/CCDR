...
TIMEOUT = float(os.environ.get("CCDR_HTTP_TIMEOUT", "30"))
...
def _clean_header_cell(x: Any, fallback: str | None = None) -> str:
...
def _unique_column_names(names: Iterable[Any]) -> list[str]:
    out: list[str] = []
    seen: dict[str, int] = {}
    for i, name in enumerate(names):
        base = _clean_header_cell(name, fallback=f"col_{i}")
        if not base:
            base = f"col_{i}"
        n = seen.get(base, 0)
        seen[base] = n + 1
...
def _read_ragged_delimited(raw: bytes, path: Path) -> pd.DataFrame | None:
    """Read ragged public CSV/TXT exports that pandas rejects."""
    ...
    delim = max(delim_scores, key=delim_scores.get)
    if delim_scores[delim] == 0:
        return None
    try:
    ...
    header = _unique_column_names(padded[0])
    body = padded[1:]
    try:
...
def read_public_table(path: str | Path) -> pd.DataFrame:
    """Robustly read a downloaded public machine-readable table.

    Besides normal CSV/XLS/XLSX parsing, this handles ragged CSV exports and
    the transposed HDB layout used by the public OSF HDB files.  It deliberately
    rejects HTML/metadata downloads so tests do not treat web pages as data.
    """
    path = Path(path)
    raw = path.read_bytes()
    head = raw[:512].lstrip()
    ...
    for kwargs in [
        {"sep": None, "engine": "python", "encoding": "utf-8-sig", "on_bad_lines": "skip"},
        {"sep": ",", "engine": "python", "encoding": "utf-8-sig", "on_bad_lines": "skip"},
        {"sep": "\t", "engine": "python", "encoding": "utf-8-sig", "on_bad_lines": "skip"},
        {"sep": ";", "engine": "python", "encoding": "utf-8-sig", "on_bad_lines": "skip"},
        {"sep": r"\s+", "engine": "python", "encoding": "utf-8-sig", "on_bad_lines": "skip"},
    ]:
        try:
            ...
            parse_errors.append(f"{kwargs.get('sep')!r}: {type(e).__name__}: {e}")
            continue

    ragged = _read_ragged_delimited(raw, path)
    if ragged is not None and len(ragged.columns) >= 2 and len(ragged) >= 1:
        return ragged

    text = raw[:4096].decode("utf-8-sig", errors="ignore")
    raise DownloadError(
        f"Downloaded resource is not a parseable CSV/TSV/Excel table: {path}; "
        f"first bytes={text[:120]!r}; parser_errors={parse_errors[:4]!r}"
    )
...
def _cell_text(x: Any) -> str:
    """Robust scalar-to-text helper for pandas object/extension dtypes.

    Some pandas builds can still feed floats/NA sentinels into Series.map after
    astype(str), which caused FR4/FR7 to crash on public HDB matrices. Keep all
    header/layout probing on plain Python values instead of relying on pandas
    string conversion semantics.
    """
    return _clean_header_cell(x)


def _nonempty_cell_count(values: Iterable[Any]) -> int:
    return sum(bool(_cell_text(v)) for v in values)


def _maybe_transpose_public_matrix(df: pd.DataFrame, candidate_terms: list[str]) -> pd.DataFrame:
    """Convert HDB-style variable-by-row matrices to ordinary row records.

    Public HDB OSF files can be shaped as:
        ,1,2,3,...
        TAUTH,0.1,0.2,...
        AMIN,0.4,0.5,...
    Tests need the transpose: one row per shot and columns TAUTH/AMIN/...

    Patch p16: avoid pandas `.astype(str).map(...)` because the user's pandas
    build produced float values inside the mapper. Also allow a transpose when
    either the data columns are numeric shot labels or the first column strongly
    looks like variable names; this handles more HDB export variants.
    """
    if df.empty or df.shape[1] < 4 or df.shape[0] < 4:
        return df

    first_values = [_cell_text(v) for v in df.iloc[:, 0].to_numpy(dtype=object)]
    joined_first_col = " ".join(v.lower() for v in first_values[:200] if v)
    term_hit_in_first_col = any(t.lower() in joined_first_col for t in candidate_terms)

    col_names = [_cell_text(c) for c in df.columns]
    numeric_named = sum(1 for c in col_names[1:min(len(col_names), 50)] if re.fullmatch(r"\d+(?:\.0)?", c))
    many_numeric_shot_cols = numeric_named >= max(3, min(25, len(col_names) - 1) // 2)

    # Do not transpose ordinary record tables that already have recognizable columns.
    if not term_hit_in_first_col and any(best_matching_column_fuzzy(df, [t]) is not None for t in candidate_terms):
        return df
    if not term_hit_in_first_col and not many_numeric_shot_cols:
        return df

    names: list[str] = []
    rows: list[list[Any]] = []
    for _, row in df.iterrows():
        values_all = row.to_numpy(dtype=object).tolist()
        if not values_all:
            continue
        name = _cell_text(values_all[0])
        if not name or re.fullmatch(r"\d+(?:\.0)?", name):
            continue
        values = values_all[1:]
        if _nonempty_cell_count(values) < 2:
            continue
        names.append(name)
        rows.append(values)
    ...
    if len(names) < 3:
        return df
    if not any(any(t.lower() in n.lower() for t in candidate_terms) for n in names):
        return df

    max_len = max(len(r) for r in rows)
    padded_rows = [r + [np.nan] * (max_len - len(r)) for r in rows]
    arr = np.array(padded_rows, dtype=object).T
    out = pd.DataFrame(arr, columns=_unique_column_names(names))
    out = out.dropna(how="all").reset_index(drop=True)
    return out

def repair_header_if_needed(df: pd.DataFrame, candidate_terms: list[str], max_rows: int = 8) -> pd.DataFrame:
    """Repair common public-table header/layout problems.

    Handles both real headers hidden in early rows and the transposed public HDB
    matrix layout where variable names are in the first column.
    """
    df = _maybe_transpose_public_matrix(df, candidate_terms)
    if any(best_matching_column_fuzzy(df, [t]) is not None for t in candidate_terms):
        return df
    for i in range(min(max_rows, len(df))):
        vals = [_clean_header_cell(v, fallback=f"col_{j}") for j, v in enumerate(df.iloc[i].tolist())]
        joined = " ".join(vals).lower()
        if any(t.lower() in joined for t in candidate_terms):
            new = df.iloc[i+1:].copy()
            new.columns = _unique_column_names(vals)
