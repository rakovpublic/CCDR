...
from _common_public_data import ensure_dir, github_find_blob, github_raw, json_dump, structured_report, cached_download, TIMEOUT
...
def figshare_collection_files(collection_id: int) -> list[dict[str, Any]]:
    ...
    try:
        meta = requests.get(f'https://api.figshare.com/v2/collections/{collection_id}', timeout=TIMEOUT).json()
        articles_url = f'https://api.figshare.com/v2/collections/{collection_id}/articles'
        articles = requests.get(articles_url, timeout=TIMEOUT).json()
        for art in articles if isinstance(articles, list) else []:
            aid = art.get('id') or art.get('article_id')
            if not aid:
                continue
            detail = requests.get(f'https://api.figshare.com/v2/articles/{aid}', timeout=TIMEOUT).json()
...
def main():
    analyses=[]; downloaded=[]; extra_sources=[]; source_errors=[]
    ...
    # If the primary GitHub source itself is unreachable, skip this secondary probe
    # so offline/blocked runs return one clean source_unavailable report quickly.
    ...
    if source_errors and not analyses:
    ...
    report = structured_report(
        'MAT1','ok' if analyses else ('source_unavailable' if source_errors else 'no_machine_readable_tables'),
