# T1: CDT Spectral Dimension Oscillation δd ~ ν ~ 10⁻³

## Prediction
The spectral dimension d_s of the CDT C-phase oscillates around its
mean value with amplitude δd ~ ν ~ 10⁻³, where ν is the RVM running
vacuum coefficient. This is the first quantum-gravity-to-cosmology bridge.

## Hardware Requirements
- CPU: any modern multi-core (i9 ideal)
- RAM: 4 GB sufficient for 2D, 16 GB for 3D
- GPU: not needed
- Disk: minimal (<1 GB)
...
```
No external packages beyond standard Python scientific stack.

## Data
Self-generated via Monte Carlo. No external data needed.

## Method

### 2D CDT Monte Carlo (Immediate — hours)

The 2D CDT model: triangulate a 2D spacetime torus with N triangles.
Pachner moves (2-2, 1-3, 3-1) explore the configuration space.
...
def run_T1_test(N_configs=500, T=20, N_spatial=25):
    ...
    of CDT configurations and look for oscillations with amplitude ~ 10⁻³.
    """
    print("=" * 60)
    ...
    print(f"⟨d_s⟩ at large σ: {ds_mean[valid][-5:].mean():.3f}")
    print(f"Residual RMS (δd): {delta_d:.5f}")
    ...
    if significance > 3:
        print("✓ OSCILLATION DETECTED at > 3σ")
    elif significance > 2:
        print("~ MARGINAL detection (2-3σ) — increase N_configs")
    else:
        print("✗ No significant oscillation detected")
        configs_needed = int(N_configs * (3 / max(significance, 0.1))**2)
        print(f"  Need ~{configs_needed} configs for 3σ at this amplitude")
...
- **Success**: δd = (1.0 ± 0.5) × 10⁻³ at > 3σ
- **Marginal**: δd detected at 2–3σ — need more configs
- **Failure**: δd < 10⁻⁴ at 5σ sensitivity — CDT-CCDR link falsified

## What to Do with Results
- If δd ~ 10⁻³: compare with ν from DESI (P01). If they match, first quantum-gravity-to-cosmology bridge.
- If δd ≠ 10⁻³: the CDT oscillation amplitude is not ν. CCDR structural link needs revision.
