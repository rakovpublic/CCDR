# SM-D5: Koide Formula Verification and Quark Extensions

## Prediction
Q = (m_e + m_μ + m_τ) / (√m_e + √m_μ + √m_τ)² = 2/3 (exact, from C₆ᵥ hexagonal symmetry).
Quark analogues: Q_quarks = 2/3 + O(α_s/π) with calculable QCD corrections.

## Hardware: Any machine. Pure arithmetic.
...
## Data
PDG 2024 mass values (pdg.lbl.gov, public).
...
# Quark MSbar masses at μ = 2 GeV (light quarks) and pole masses (heavy)
QUARKS_MSBAR = {
    'u': 0.00216,   # ± 0.00049
    'd': 0.00467,   # ± 0.00048
    's': 0.0934,    # ± 0.0086
    'c': 1.27,      # ± 0.02 (at μ = m_c)
    'b': 4.18,      # ± 0.03 (at μ = m_b)
    't': 172.69,    # ± 0.30 (pole mass)
}

# Running masses at μ = M_Z = 91.1876 GeV (for uniform comparison)
QUARKS_MZ = {
    'u': 0.00127,
    'd': 0.00284,
    's': 0.0555,
...
def koide_Q(m1, m2, m3):
    """Compute the Koide ratio Q = (m1+m2+m3) / (√m1+√m2+√m3)²."""
    numerator = m1 + m2 + m3
    denominator = (np.sqrt(m1) + np.sqrt(m2) + np.sqrt(m3))**2
    return numerator / denominator

def qcd_correction(alpha_s, n_loops=1):
    """QCD correction to Koide for quarks: δQ = c × α_s/π."""
    if n_loops == 1:
...
def main():
    ...
    if abs(Q_lep - 2/3) < 0.001:
        print(f"  ✓ Koide Q = 2/3 confirmed to {-np.log10(abs(Q_lep-2/3)):.0f} significant figures")
    ...
    print(f"  Predicted: 2/3 + α_s CF/π = {Q_up_predicted:.6f}")
    ...
    for n1, n2, n3, masses in triplets:
        ...
        print(f"  ({n1}, {n2}, {n3}): Q = {Q:.6f}, Δ = {Q - 2/3:+.6f}")

    # 5. Scale dependence: run quarks to different μ and check Q
    print(f"\nSCALE DEPENDENCE (Q_up at different μ):")
    # Running mass: m(μ) = m(M_Z) × [α_s(μ)/α_s(M_Z)]^(γ₀/β₀)
    # γ₀ = 8, β₀ = 23/3 for n_f = 5
    gamma0 = 8.0
    beta0 = 23.0 / 3.0
    mu_values = [2, 5, 10, 50, 91.2, 200, 1000]  # GeV
    for mu in mu_values:
        # Approximate α_s running
        ...
        print(f"  μ = {mu:6.1f} GeV: α_s = {alpha_s:.4f}, "
    ...
    def Q_minus_twothirds(log_mu):
        mu = np.exp(log_mu)
        alpha_s = 0.1179 / (1 + 0.1179 * beta0 / (2*np.pi) * np.log(mu/91.2))
        if alpha_s <= 0 or alpha_s > 1: return 1.0
        ratio = (alpha_s / 0.1179)**(gamma0 / (2*beta0))
        Q = koide_Q(QUARKS_MZ['u']*ratio, QUARKS_MZ['c']*ratio, QUARKS_MZ['t']*ratio)
        return Q - 2/3
    ...
    for log_mu in np.linspace(0, 15, 100):
        ...
        if abs(val) < 0.001:
            ...
            print(f"  Q ≈ 2/3 at μ ≈ {mu_star:.0f} GeV (hexagonal BZ scale?)")

if __name__ == '__main__':
    main()
```

## Expected: Q_leptons = 0.666661 (matches 2/3 to 0.0007%)
## Timeline: 1 hour
